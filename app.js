...
function closeModal() {
  ...
  gallery.removeEventListener("keydown", arrowRight);
  gallery.removeEventListener("keydown", arrowLeft);
}
...
function rightNext() {
  const activeImg = galleryItems.findIndex(
...
function leftNext() {
  ...
  let index = activeImg ? activeImg : galleryItems.length - 1;
  if (index > 0) {
    index -= 1;
  } else {
